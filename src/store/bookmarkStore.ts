...
interface BookmarkState {
  ...
  createBookmark: (bookmark: Omit<Bookmark, 'id' | 'dateAdded'>) => Promise<chrome.bookmarks.BookmarkTreeNode>;
...
export const useBookmarkStore = create<BookmarkState>()((set, get) => ({
  ...
  checkPermission: async () => {
    try {
      ...
      set((state) => ({ permissionState: { ...state.permissionState, hasPermission } }));
  ...
  requestPermission: async () => {
    ...
    try {
      ...
      set({ permissionState: { hasPermission: granted, isRequesting: false } });
      return granted;
    } catch (error) {
  ...
  moveBookmarkOptimized: async (draggedId, targetId) => {
    try {
      ...
      let newIndex = dragged.index > target.index ? target.index : target.index + 1;
          ...
          folders: state.folders.map((folder) => {
            if (
              ...
              if (draggedChildIndex !== -1 && targetChildIndex !== -1) {
                const [movedBookmark] = newChildren.splice(draggedChildIndex, 1);
